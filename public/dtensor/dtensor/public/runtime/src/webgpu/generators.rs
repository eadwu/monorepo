...
for (var i = 0u; i < {input_metadata_variable}.dimension; i++) {{
    ...
    let index_at_dimension = ({output_variable}_temp / input_contiguous_stride) % input_shape;

    let mapped_stride = {mapped_metadata_variable}.metadata[{input_metadata_variable}.stride_offset + i];
