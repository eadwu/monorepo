...
@group(0) @binding(1) var<storage, read> left_rank: u32;
...
@group(1) @binding(1) var<storage, read> right_rank: u32;
...
@group(2) @binding(1) var<storage, read> sum_size: u32;
...
fn main(@builtin(global_invocation_id) global_id: vec3u) {
  ...
  let left_stride_offset = left_rank;
  let left_contiguous_stride_offset = left_rank + left_rank;

  var contiguous_offset = index;
  var mapped_offset = 0u;

  for (var i = 0u; i < left_rank; i++) {
    let contiguous_offset_at_dimension = left_shape[left_contiguous_stride_offset + i];
    let stride_offset_at_dimension = left_shape[left_stride_offset + i];

    let offset_at_dimension = contiguous_offset / contiguous_offset_at_dimension;
    let offset = stride_offset_at_dimension * offset_at_dimension;

    mapped_offset = mapped_offset + offset;
    contiguous_offset = contiguous_offset % contiguous_offset_at_dimension;
  }
  ...
  let right_stride_offset = right_rank;
  let right_contiguous_stride_offset = right_rank + right_rank;

  contiguous_offset = index;
  mapped_offset = 0u;

  for (var i = 0u; i < right_rank; i++) {
    let contiguous_offset_at_dimension = right_shape[right_contiguous_stride_offset + i];
    let stride_offset_at_dimension = right_shape[right_stride_offset + i];
  ...
  sum[index] = left[left_index] + right[right_index];
}
